...
public class PdfRenderer extends BasicRenderer {
	...
	public PdfRenderer(Resource ir, MimeType mt) {
		super(ir, mt);
	 ...
	 * 
	 * @param from
	 *            File di origine per il quale si richiede la conversione in PDF/A
	 * 
	 *            See
	 *            it.tredi.fcs.command.conversion.OpenOfficeConversionExecutor.java
	 */
	private DocumentFormat getPdfaDocumentFormat(File from) {
		...
		if (from != null) {
			 ...
			 * properties.put("FilterName", filterName); properties.put("FilterData",
			 * filterData);
			 */

			format = DocumentFormat.builder().extension("pdf").mediaType("application/pdf").name("PDF/A")
					.storeProperty(docFamily, "FilterName", filterName)
					.storeProperty(docFamily, "FilterData", filterData).build();
	...
	public void run() {
		...
		try {
			...
			FileUtils.copyInputStreamToFile(ir.getInputStream(), officeFile);
			// Create file according to out mime type.

			pdfFile = File.createTempFile("pdf-", ".pdf");
			if (logger.isDebugEnabled())
				logger.debug("PdfConverter.run(): from " + officeFile.getAbsolutePath() + " to "
			...
			if (logger.isInfoEnabled())
				logger.info("PdfConverter.run(): from " + officeFile.getAbsolutePath() + " to "
						+ pdfFile.getAbsolutePath());
			...
			if (logger.isInfoEnabled())
				logger.info("PdfConverter.run(): conversion tooks " + (System.currentTimeMillis() - startTime)
						+ " millis.");
