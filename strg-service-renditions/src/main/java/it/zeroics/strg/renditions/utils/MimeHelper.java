...
public class MimeHelper {
	...
	public static final String METADATA_MIMETYPE = "application/json";
	...
	public Boolean isMeta() {
		try {
			if ( parms.get(METADATA_KEY).equals(TRUE_VALUE) ) return true ; 
		}
		catch(Exception e) {}
		return false ;
	}
	public static Boolean isMeta(MimeType m) {
		try {
			if ( m.getParameter(METADATA_KEY).equals(TRUE_VALUE) ) return true ; 
		}
		catch(Exception e) {}
		return false ;
	}
	public static boolean justMeta(MimeType mime) {
        ...
        if ( mime.isCompatibleWith(MimeType.valueOf(METADATA_MIMETYPE)) ) {
        	if ( isMeta(mime) ) {
        		return true ;
        	}
        }
