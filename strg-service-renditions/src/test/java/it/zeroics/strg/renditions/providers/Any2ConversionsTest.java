			...
			Context("#isCapable", () -> {
				It("Must be capable only in some cases", () -> {
					...
					assertThat(
							...
							is(true));
					assertThat(provider
							.isCapable("application/vnd.oasis.opendocument.text", MimeHelper.CAPABILITY_MIMETYPE)
							.isBest(), is(true));
					...
					assertThat(
							...
							is(true));
					assertThat(provider.isCapable(
							"application/vnd.openxmlformats-officedocument.wordprocessingml.document",
							"application/pdf"), is(RenditionCapability.NOT_CAPABLE));
			...
			Context("#convert", () -> {
				...
				Context("#get service capabilities", () -> {
					It("Capabilityes for docx", () -> {
						...
						JsonNode jn = om.readTree(c.callConverterFromFileName("sample-docx.docx.pdf", "application/pdf",
								MimeHelper.CAPABILITY_MIMETYPE, provider).getInputStream());
