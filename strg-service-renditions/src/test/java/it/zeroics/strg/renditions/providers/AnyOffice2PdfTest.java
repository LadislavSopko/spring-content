		...
		Describe("RenditionProviders: AntOffice2Pdf", () -> {
			...
			Context("#isCapable", () -> {
				It("Must be capable only in some cases", () -> {
					...
					assertThat(provider.isCapable("what/the_fuck", "application/pdf"),
							is(RenditionCapability.NOT_CAPABLE));
					...
					assertThat(provider.isCapable("image/dicom", "application/pdf"),
							is(RenditionCapability.NOT_CAPABLE));
					assertThat(provider.consumes("application/msword"), is(true));
					assertThat(provider.isCapable("application/msword", "application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(provider.consumes("application/vnd.ms-powerpoint"), is(true));
					assertThat(provider.isCapable("application/vnd.ms-powerpoint", "application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(provider.consumes("application/vnd.ms-excel"), is(true));
					assertThat(provider.isCapable("application/vnd.ms-excel", "application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(provider.consumes("application/vnd.oasis.opendocument.text"), is(true));
					assertThat(provider.isCapable("application/vnd.oasis.opendocument.text", "application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(provider.consumes("application/vnd.oasis.opendocument.spreadsheet"), is(true));
					assertThat(provider.isCapable("application/vnd.oasis.opendocument.spreadsheet", "application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(provider.consumes("application/vnd.oasis.opendocument.presentation"), is(true));
					assertThat(provider.isCapable("application/vnd.oasis.opendocument.presentation", "application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(
							provider.consumes(
					...
					assertThat(provider
							.isCapable("application/vnd.openxmlformats-officedocument.wordprocessingml.document",
									"application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(provider.consumes("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"),
							is(true));
					assertThat(provider.isCapable("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
							"application/pdf").isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(
							provider.consumes(
									"application/vnd.openxmlformats-officedocument.presentationml.presentation"),
							is(true));
					assertThat(provider
							.isCapable("application/vnd.openxmlformats-officedocument.presentationml.presentation",
									"application/pdf")
							.isBetterThan(RenditionCapability.NOT_CAPABLE), is(true));
					assertThat(
							provider.consumes("application/vnd.openxmlformats-officedocument.presentationml.slideshow"),
							is(true));
					assertThat(
							provider.isCapable("application/vnd.openxmlformats-officedocument.presentationml.slideshow",
									"application/pdf").isBetterThan(RenditionCapability.NOT_CAPABLE),
							is(true));
					assertThat(provider.consumes(MimeHelper.CAPABILITY_MIMETYPE), is(false));
					assertThat(
							provider.isCapable("application/vnd.oasis.opendocument.text",
									MimeHelper.CAPABILITY_MIMETYPE).isBetterThan(RenditionCapability.NOT_CAPABLE),
							is(false));
				});
			...
			Context("#convert", () -> {
				Context("#docx to pdf", () -> {
					It("Must convert docx to pdf", () -> {
						...
						Resource pdfResource = c.callConverterFromFileName("sample-docx.docx",
								"application/vnd.openxmlformats-officedocument.wordprocessingml.document",
								"application/pdf", provider);
						assertThat(pdfResource, is(not(nullValue())));

						MostAny2Txt txtProvider = new MostAny2Txt();
						/*
						 * Text extraction from PDF differs from text extraction from doc converted to
						 * PDF. Must use a different pattern file
						 */
						assertThat(
								c.compareAsByteArray(
										c.callConverterFromInputStream(pdfResource, "sample-doc.docx.pdf",
												"application/pdf", "text/plain", txtProvider),
										"sample-docx.docx.pdf.txt"),
								is(true));
					});
				});
			});
